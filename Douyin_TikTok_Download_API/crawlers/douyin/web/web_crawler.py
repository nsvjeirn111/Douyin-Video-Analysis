...
from crawlers.douyin.web.models import (
    BaseRequestModel, ChannelFeed, GeneralSearch, VideoSearch,
    LiveRoomRanking, PostComments,
...
class DouyinWebCrawler:
    ...
    # 获取综合搜索结果
    async def fetch_general_search_result(self, keyword: str, offset: int = 0, count: int = 20,
        ...
        async with base_crawler as crawler:
            ...
            endpoint = BogusManager.xb_model_2_endpoint(
                DouyinAPIEndpoints.GENERAL_SEARCH, params.dict(), kwargs["headers"]["User-Agent"]
            )
            response = await crawler.fetch_get_json(endpoint)
        return response

    # 获取视频搜索结果
    async def fetch_video_search_result(self, keyword: str, offset: int = 0, count: int = 20,
                                        sort_type: str = "1", publish_time: str = "0",
                                        filter_duration: str = "0"):
        kwargs = await self.get_douyin_headers()
        base_crawler = BaseCrawler(proxies=kwargs["proxies"], crawler_headers=kwargs["headers"])
        async with base_crawler as crawler:
            params = VideoSearch(keyword=keyword, offset=offset, count=count,
                                 sort_type=sort_type, publish_time=publish_time,
                                 filter_duration=filter_duration)
            endpoint = BogusManager.xb_model_2_endpoint(
    ...
    # 获取视频频道Feed
    async def fetch_channel_feed(self, channel_id: str = "", count: int = 20, cursor: int = 0):
        kwargs = await self.get_douyin_headers()
        base_crawler = BaseCrawler(proxies=kwargs["proxies"], crawler_headers=kwargs["headers"])
        async with base_crawler as crawler:
            params = ChannelFeed(channel_id=channel_id, count=count, cursor=cursor)
            endpoint = BogusManager.xb_model_2_endpoint(
                DouyinAPIEndpoints.DOUYIN_VIDEO_CHANNEL, params.dict(), kwargs["headers"]["User-Agent"]
            )
            response = await crawler.fetch_get_json(endpoint)
        return response
