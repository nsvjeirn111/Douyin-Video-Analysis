...
# 获取抖音热榜数据
@router.get("/fetch_hot_search", response_model=ResponseModel, summary="获取抖音热榜数据/Get Douyin hot search list")
...
# 获取综合搜索结果
@router.get("/fetch_general_search", response_model=ResponseModel, summary="获取综合搜索结果/Get general search result")
async def fetch_general_search(request: Request,
                               keyword: str = Query(example="美食", description="搜索关键词/Search keyword"),
                               offset: int = Query(default=0, example=0, description="偏移量/Offset"),
                               count: int = Query(default=20, example=20, description="数量/Count"),
                               sort_type: str = Query(default="0", example="0", description="排序方式: 0-综合 1-最多点赞 2-最新发布"),
                               publish_time: str = Query(default="0", example="0", description="发布时间筛选: 0-不限 1-一天内 7-一周内 180-半年内"),
                               filter_duration: str = Query(default="0", example="0", description="时长筛选: 0-不限 1-1分钟以内 2-1-5分钟 3-5分钟以上")):
    try:
        data = await DouyinWebCrawler.fetch_general_search_result(keyword, offset, count,
...
# 获取视频搜索结果
@router.get("/fetch_video_search", response_model=ResponseModel, summary="获取视频搜索结果/Get video search result")
async def fetch_video_search(request: Request,
                             keyword: str = Query(example="搞笑", description="搜索关键词/Search keyword"),
                             offset: int = Query(default=0, example=0, description="偏移量/Offset"),
                             count: int = Query(default=20, example=20, description="数量/Count"),
                             sort_type: str = Query(default="1", example="1", description="排序方式: 0-综合 1-最多点赞 2-最新发布"),
                             publish_time: str = Query(default="0", example="0", description="发布时间筛选"),
                             filter_duration: str = Query(default="0", example="0", description="时长筛选")):
    try:
        data = await DouyinWebCrawler.fetch_video_search_result(keyword, offset, count,
                                                                  sort_type, publish_time, filter_duration)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
...
# 获取视频频道Feed
@router.get("/fetch_channel_feed", response_model=ResponseModel, summary="获取视频频道Feed/Get channel feed")
async def fetch_channel_feed(request: Request,
                             channel_id: str = Query(default="", example="", description="频道ID/Channel ID, 留空获取默认推荐"),
                             count: int = Query(default=20, example=20, description="数量/Count"),
                             cursor: int = Query(default=0, example=0, description="游标/Cursor")):
    try:
        data = await DouyinWebCrawler.fetch_channel_feed(channel_id, count, cursor)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
